...
function computeSignals(state: MissionState) {
  ...
  if (commitCount >= 8 && pack.commits.length === new Set(pack.commits.map((c) => c.author)).size) {
    // single author across many commits — fine but skip
  }
  ...
  // Heuristic score: start at 75, subtract risks, add positives.
  let score = 75;
  score -= risk.length * 8;
...
export async function runAuthenticity(state: MissionState): Promise<Handoff<AuthenticityOutput>> {
  ...
  let out: AuthenticityOutput;
  let tin = 0, tout = 0;
  const baseline = fallback(state);

  if (isMockMode()) {
    out = { ...baseline, score_source: state.mock_mode ? "mock" : "heuristic" };
  } else {
...
Return JSON now.`;
    try {
      ...
      tin = r.inputTokens;
      tout = r.outputTokens;
      const parsed = extractJson<AuthenticityOutput>(r.text);
      out = parsed ? { ...parsed, score_source: "llm" } : { ...baseline, score_source: "heuristic" };
    } catch {
      out = { ...baseline, score_source: "heuristic" };
    }
  }
